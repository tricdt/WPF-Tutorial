...
namespace DevExpress.Xpf.Native.Prism800 {
	public class NavigationFrameAdapterImpl {
		class Adapter : AdapterBase<NavigationFrame> {
			bool updating = false;
			public Adapter(IRegionRuntimeWrapper region, NavigationFrame target) : base(region, target) {
			...
			public object Content {
				get { return content; }
				set {
					if(content != value && !updating && value != null) {
			...
			protected override void OnActiveViewsCollectionChanged(object sender, NotifyCollectionChangedEventArgs e) {
				...
				try {
					Target.Dispatcher.InvokeAsync(() => {
						var active = ((IEnumerable)Region.ActiveViews.Object).OfType<object>().FirstOrDefault();
						Target.Navigate(active);
						isNavigating = false;
					});
				} finally {
					updating = false;
		...
		public NavigationFrameAdapterImpl(int prismVersion) {
		}
		public void Adapt(IRegionRuntimeWrapper region, NavigationFrame regionTarget) {
